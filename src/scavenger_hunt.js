...
$(document).ready(function() {
  ...
  function clickprize() {
    if (userID !== 0) {
      ...
      var user_id = proboards.data("user").id;
      ...
      if (typeof dataCheck !== 'undefined') {
        ...
        var leaderDisplay = pb.plugin.key('sh_leaderboard').get() || [];
        
        leaderDisplay.sort((a, b) => {
            if(a.count > b.count){
                return -1;
            }
            if(a.count < b.count){
        ...
        for(i = 0; i < leaderDisplay.length; ++i) {
            var safe_amount = parseInt(leaderDisplay[i].count, 10) || 0;
            var safe_name = pb.text.escape_html(leaderDisplay[i].name);
            var safe_id = pb.text.escape_html(parseInt(leaderDisplay[i].id, 10) || 0);
            
            html += "<div class='shlb_entry-" + i + "'>";
            html += i + ") <a href='/user/" + safe_id + "'>" + safe_name + "</a> (" + safe_count + ")";
            html += "</div>";
        }
        
        $("#sh_leaderboard").html(html);  
